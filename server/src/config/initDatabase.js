  ...
  try {
    ...
    const [superadminRole, superadminCreated] = await Role.findOrCreate({
      ...
      defaults: {
        ...
        name: "superadmin",
        description:
          "Super Administrator Role: Users have full access and control",
      },
    });
    ...
    if (error.name === "SequelizeAccessDeniedError") {
      console.error(
        "💡 Database access denied. Check your username and password in .env"
      );
    } else if (
      ...
      console.error(
        `   CREATE DATABASE ${process.env.DB_NAME || "fullstack_app"};`
      );
